defmodule BoggleEngine.Board.Solver do
  ...
  ## Options
    ...
    * `:lexicon` - Default is built-in lexicon. Accepts custom lexicon
      generated by `Lexicon`.
  """
  @spec solve(board, rule, integer, keyword) :: [word]
  def solve(board, rule, minimum_word_length, options \\ []) do
    size = Board.get_size(board)
    neighbors = &(Neighbor.get_neighbors(&1, size, rule))

    lexicon =
      if Keyword.has_key?(options, :lexicon) do
        Keyword.get(options, :lexicon)
      else
        :erlang.binary_to_term(@lexicon_binary)
      end
